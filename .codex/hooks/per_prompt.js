...
function classifyKind(text = '', role = 'user') {
...
async function main() {
  const { text, role = 'user', kind: kindArg, language, tool, domain, level, followup, force } = parseArgs();
  if (!text) {
  ...
  const tags = [
    ...buildTags({
      ...
      level,
      lifecycle: 'prompt',
      extra: [fpTag, `role:${role}`, `kind:${kind.toLowerCase()}`, ...followupTag],
    }),
  ...
  if (!force) {
    try {
      ...
      console.warn(`dedupe search failed: ${err.message || err}`);
    }
  }
