...
function createTaskService(deps) {
    ...
    return {
        async list(groupIds, limit, defaultRepo, defaultAssignee) {
          ...
          LIMIT ${limit}
        `;
                const records = await neo4jClient.query(cypher);
                // Parse tasks from episodic records
                const tasks = records.map((r) => {
                    let taskObj = null;
                    try {
                        if (r.content)
                            taskObj = JSON.parse(r.content);
                    }
                    catch {
                        /* ignore parse errors */
                    }
        ...
        async add(title, groupId, options) {
            const taskObj = {
                type: 'task',
                title,
            ...
            const result = await mcpClient.rpcCall('add_memory', params);
            return {
                status: 'ok',
                action: 'add',
                task: taskObj,
        ...
        async update(title, groupId, options) {
            const taskObj = {
                type: 'task',
                title,
                status: options.status || 'todo',
