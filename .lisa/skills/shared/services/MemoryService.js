 ...
 */
function resolveTag(text, options) {
...
function createMemoryService(deps) {
    ...
    return {
        async load(groupIds, query, limit) {
          ...
          `;
                ...
                const records = await neo4jClient.query(cypher, query && query !== '*' ? { query } : {});
        ...
        async add(text, groupId, options) {
