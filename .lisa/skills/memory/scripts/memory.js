...
async function main() {
    ...
    const { resolveTag } = await Promise.resolve().then(() => __importStar(require('../../common/type-mappings')));
    ...
    const { createMemoryService, createMemoryCliService } = await Promise.resolve().then(() => __importStar(require('../../shared/services')));
    const env = loadEnv();
    ...
    const memoryService = createMemoryService({ neo4jClient, mcpClient, zepClient });
