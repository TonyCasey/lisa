...
If the commit completes ticket (JIRA_PREFIX-XXXX), also run:
```bash
node .agents/skills/tasks/scripts/tasks.js add "JIRA_PREFIX-XXXX: COMPLETED - <description>" --cache --status done
```
...
node .agents/skills/memory/scripts/memory.js add "FEATURE: [JIRA_PREFIX-XXXX] Added Excel brandlist parser with unit tests" --cache --type milestone

# Step 2: Update task status
node .agents/skills/tasks/scripts/tasks.js add "JIRA_PREFIX-XXXX: COMPLETED - Excel brandlist parser utility created, PR #XXXX" --cache --status done
```
