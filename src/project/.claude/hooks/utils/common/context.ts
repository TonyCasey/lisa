 ...
 *
 * Aliases support:
 * - PROJECT_ALIASES in .lisa/.env (comma-separated)
...
/**
 * Write or update PROJECT_NAME in .lisa/.env
 * Preserves existing entries
 */
function writeProjectName(projectName: string, detectedFrom: string): boolean {
  const envPath = path.join(process.cwd(), ENV_FILE);
  const agentsDir = path.join(process.cwd(), '.lisa');
...
/**
 * Get the project name, initializing .lisa/.env if needed
 */
function detectRepo(): string {
  // 1. Check for PROJECT_NAME in .lisa/.env
...
function repoTags({ repo, branch }: RepoTagsOptions = {}): string[] {
  const tags: string[] = [];
