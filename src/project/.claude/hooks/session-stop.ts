...
async function main(): Promise<void> {
  const log = createHookLogger('session-stop');
  
  // 1. Read input from stdin
  const input = await readStdin();
  
  log.debug('Session stop hook triggered', { 
  ...
  if (input.stop_hook_active) {
    log.debug('Skipping - stop hook already active');
  ...
  try {
    ...
    worker.unref();
    log.debug('Spawned background worker', { workerPath });
  } catch (err) {
    // Spawn failed - exit silently, don't block CLI
    log.error('Failed to spawn worker', { error: (err as Error).message });
  }
