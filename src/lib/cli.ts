...
import {
  ...
  SessionStopHookHandler,
  UserPromptSubmitHookHandler,
} from './application/handlers/hooks';
...
hookCmd
  ...
  .action(async () => {
    const handler = new SessionStartHookHandler();
    await handler.execute(process.stdin, process.stdout, process.stderr);
  });
...
hookCmd
  ...
  .action(async () => {
    const handler = new SessionStopHookHandler();
    await handler.execute(process.stdin, process.stdout, process.stderr);
  });
...
hookCmd
  ...
  .action(async () => {
    const handler = new UserPromptSubmitHookHandler();
    await handler.execute(process.stdin, process.stdout, process.stderr);
  });
