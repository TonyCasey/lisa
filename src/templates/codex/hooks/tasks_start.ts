...
// Mark a task as in-progress using its task_id.
...
async function main() {
  ...
  if (!id && !num) {
    console.error('tasks_start requires --id <task_id> or --num <task_num>');
    process.exit(1);
  }
  ...
  console.log(formatted || `Marked task ${id || num} as in-progress.`);
}
